...
void Sys_Init (void)
{
	...
	{
		Sys_Error ("fxHexen II requires at least Win95 or NT 4.0");
	}
...
void Sys_Error (char *error, ...)
{
	...
	{
		snprintf (text2, sizeof(text2), "HEXEN II ERROR: %s\n", text);
	...
	else
	{
		...
		{
			if (!NoMsgBox)
				MessageBox(NULL, text, "Double Hexen II Error", MB_OK | MB_SETFOREGROUND | MB_ICONSTOP);
		}
